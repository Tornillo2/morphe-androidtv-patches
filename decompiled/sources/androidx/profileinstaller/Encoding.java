...
public class Encoding {
    ...
    public static byte[] read(@NonNull InputStream inputStream, int i) throws IOException {
        ...
        while (i2 < i) {
            ...
            if (i3 < 0) {
                throw new IllegalStateException(MediaBrowserCompat$MediaBrowserImplBase$$ExternalSyntheticOutline0.m("Not enough bytes to read: ", i));
            }
            i2 += i3;
        }
        return bArr;
    ...
    public static void writeCompressed(@NonNull OutputStream outputStream, byte[] bArr) throws IOException {
        writeUInt(outputStream, bArr.length, 4);
        byte[] bArrCompress = compress(bArr);
        writeUInt(outputStream, bArrCompress.length, 4);
