...
public final class OggPacket {
    ...
    public boolean populate(ExtractorInput extractorInput) throws IOException {
        ...
        while (!this.populated) {
            if (this.currentSegmentIndex < 0) {
                if (!this.pageHeader.skipToNextPage(extractorInput, -1L) || !this.pageHeader.populate(extractorInput, true)) {
                    return false;
                }
                OggPageHeader oggPageHeader = this.pageHeader;
            ...
            if (iCalculatePacketSize2 > 0) {
                ...
                if (!ExtractorUtil.readFullyQuietly(extractorInput, parsableByteArray2.data, parsableByteArray2.limit, iCalculatePacketSize2)) {
                    return false;
                }
    ...
    public void trimPayload() {
        ...
        parsableByteArray.reset(Arrays.copyOf(bArr, Math.max(65025, parsableByteArray.limit)), this.packetArray.limit);
    }
}
