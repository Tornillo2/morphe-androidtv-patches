...
public abstract class UnicodeEscaper extends Escaper {
    ...
    public static int codePointAt(CharSequence seq, int index, int end) {
        seq.getClass();
        if (index >= end) {
        ...
        while (index < length) {
            ...
            if (cArrEscape != null) {
                ...
                if (cArrCharBufferFromThreadLocal.length < length3) {
                    cArrCharBufferFromThreadLocal = growBuffer(cArrCharBufferFromThreadLocal, length2, (length - index) + length3 + 32);
                }
                if (i3 > 0) {
