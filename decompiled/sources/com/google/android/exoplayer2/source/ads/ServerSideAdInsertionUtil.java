...
public final class ServerSideAdInsertionUtil {
    ...
    public static AdPlaybackState correctFollowingAdGroupTimes(AdPlaybackState adPlaybackState, int i, long j, long j2) {
        long j3 = (-j) + j2;
        while (true) {
            i++;
    ...
    public static long getStreamPositionUs(Player player, AdPlaybackState adPlaybackState) {
        ...
        Timeline.Period period = currentTimeline.getPeriod(player.getCurrentPeriodIndex(), new Timeline.Period(), false);
        if (!Util.areEqual(period.adPlaybackState.adsId, adPlaybackState.adsId)) {
            return -9223372036854775807L;
        }
        if (!player.isPlayingAd()) {
            return getStreamPositionUsForContent(Util.msToUs(player.getCurrentPosition()) - period.positionInWindowUs, -1, adPlaybackState);
        }
